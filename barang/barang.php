...
<html lang="en">
	<head>
		...
		<title>Barang | UD. Az-zikra</title>
									...
									<form action="barang_act.php" method="post">
										...
										<div class="form-group">
											...
											<select class="form-control" name="nama_sales">
												...
												<?php
													$sql = mysql_query("SELECT * FROM supplier ORDER BY nama_sales ASC");
													if(mysql_num_rows($sql) != 0){
														while($data = mysql_fetch_assoc($sql)){
															echo '<option>'.$data['nama_sales'].'</option>';
														}
													}
												?>
											</select>
										...
										<div class="form-group">
											<label>Satuan</label>
											<input type="text" id="text-input" name="satuan" class="form-control" placeholder="Satuan" maxlength="20" onkeyup="validHuruf(this)" required>
										</div>
								</div>
